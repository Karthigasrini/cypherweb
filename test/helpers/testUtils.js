...
const getValues = (object, named) => {
  ...
  switch (object.type) {
      ...
      case 'buffer':
          return CypherWeb.utils.ethersUtils.arrayify(object.value);
...
const equals = (actual, expected) => {
  ...
  if (expected.buffer) {
      ...
      actual = CypherWeb.utils.ethersUtils.arrayify(actual);
  ...
  // Maybe address?
  try {
      if (CypherWeb.isAddress(actual)) {
        let actualAddress = actual;
        let expectedAddress = CypherWeb.address.toHex(expected);
