...
describe('#contract.method', function () {
    ...
    describe('#send()', function () {
        ...
        before(async function () {
            ...
            testRevert = await cypherWeb.contract().at(tx.transaction.contract_address)

            const tx2 = await broadcaster(cypherWeb.transactionBuilder.createSmartContract({
                abi: testSetValContract.abi,
                bytecode: testSetValContract.bytecode
            }, accounts.b58[0]), accounts.pks[0])
            testSetVal = await cypherWeb.contract().at(tx2.transaction.contract_address)
        })
