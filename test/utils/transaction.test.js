...
const AbiCoder = CypherWeb.utils.ethersUtils.AbiCoder;
...
describe('CypherWeb.utils.transaction', function () {
    let accounts;
    let cypherWeb;
    ...
    describe('#txCheck', function () {
        ...
        const commonAssertPb = (transaction) => {
            const transactionPb = CypherWeb.utils.transaction.txJsonToPb(transaction);
            const rawDataBytes = transactionPb.getRawData().serializeBinary();
            const txID = CypherWeb.utils.ethersUtils.sha256(rawDataBytes);
        ...
        describe('#case TransferContract', function () {
            ...
            it(`it should return true`, async function () {
                for (let param of params) {
                    const transaction = await cypherWeb.transactionBuilder.sendCyp(
                        ...param
                    );
                    const authResult =
        ...
        describe('#case TransferAssetContract', function () {
            ...
            before(async function () {
                ...
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[3], 100e6), PRIVATE_KEY);
                await broadcaster(cypherWeb.transactionBuilder.createToken(tokenOptions, accounts.b58[3]), accounts.pks[3])

                let tokenList
                await waitChainData('token', accounts.b58[3]);
                tokenList = await cypherWeb.cyp.getTokensIssuedByAddress(accounts.b58[3]);
            ...
            it(`it should return true`, async function () {
                ...
                for (let param of params) {
                    const transaction = await cypherWeb.transactionBuilder.sendToken(
                        ...param
                    );
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
        ...
        describe('#case ParticipateAssetIssueContract', function () {
            ...
            before(async function () {
                ...
                let tokenList = await cypherWeb.cyp.getTokensIssuedByAddress(accounts.b58[2])
                if (isAllowSameTokenNameApproved) {
                    tokenID = tokenList[tokenOptions.name].id
                } else {
                    tokenID = tokenList[tokenOptions.name].name
            ...
            it(`it should return true`, async function () {
                for (let param of params) {
                    const transaction = await cypherWeb.transactionBuilder.purchaseToken(
                        ...param
                    );
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
        ...
        describe('#case TriggerSmartContract', function () {
            ...
            it('should trigger smart contract successfully', async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    transaction = await cypherWeb.transactionBuilder.triggerSmartContract(
                        contractAddress,
                        functionSelector,
                        options,
                        parameter,
                    ...
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction.transaction);
                    assert.equal(authResult, true);
        ...
        describe.skip('#case WithdrawBalanceContract', function () {
            ...
            before(async () => {
                ...
                const transaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[1], 'url.cypher.network');
                await broadcaster(transaction, accounts.pks[1]);
                while (true) {
                    const tx = await cypherWeb.cyp.getTransactionInfo(transaction.txID);
                    if (Object.keys(tx).length === 0) {
        ...
        describe('#case UnfreezeBalanceV2Contract', function () {
            ...
            it('should return true', async function () {
                for (const param of params) {
                    const freezeTx = await cypherWeb.transactionBuilder.freezeBalanceV2(...param);
        ...
        describe('#case DelegateResourceContract', function () {
            ...
            before(async () => {
                ...
                const freezeTx = await cypherWeb.transactionBuilder.freezeBalanceV2(10e8, 'BANDWIDTH', accounts.b58[1]);
                await broadcaster(null, accounts.pks[1], freezeTx);
                await waitChainData('tx', freezeTx.txID);
            });
        ...
        describe('#case UnDelegateResourceContract', function () {
            ...
            it('should return true', async function () {
                for (const param of params) {
                    const delegateTx = await cypherWeb.transactionBuilder.delegateResource(...param);
        ...
        describe('#case WithdrawExpireUnfreezeContract', function () {
            ...
            before(async () => {
                ...
                const freezeTx = await cypherWeb.transactionBuilder.freezeBalanceV2(10e6, 'BANDWIDTH', ...params[1]);
                await broadcaster(null, accounts.pks[1], freezeTx);
                await waitChainData('tx', freezeTx.txID);
                const unfreezeTx = await cypherWeb.transactionBuilder.unfreezeBalanceV2(10e6, 'BANDWIDTH', ...params[1]);
        ...
        describe("#case WitnessCreateContract", async function () {
            ...
            it('should allow accounts[1] to apply for SR, it should return true', async function () {
                ...
                for (let param of params) {
                    const sendCypTransaction = await cypherWeb.transactionBuilder.sendCyp(accounts.b58[1], 11000e6);
                    await broadcaster(sendCypTransaction, PRIVATE_KEY);
                    const transaction = await cypherWeb.transactionBuilder.applyForSR(...param);
        ...
        describe('#case VoteWitnessContract', function () {
            ...
            before(async function () {
                ...
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[0], 10000e6), PRIVATE_KEY);
                await broadcaster(cypherWeb.transactionBuilder.applyForSR(accounts.b58[0], url), accounts.pks[0])
                await broadcaster(cypherWeb.transactionBuilder.freezeBalanceV2(100e6, 'BANDWIDTH', accounts.b58[1]), accounts.pks[1])
            })
            ...
            it('should allows accounts.b58[1] to vote for accounts[0] as SR, it should return true', async function () {
                ...
                for (const [votes, Permission_id] of list) {
                    const transaction = await cypherWeb.transactionBuilder.vote(
                        votes,
                        accounts.b58[1],
        ...
        describe("#case CreateSmartContract", function () {
            it('should create a smart contract with default parameters, it should return true', async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    if (i === 1) options.permissionId = 2;
                    const transaction = await cypherWeb.transactionBuilder.createSmartContract(options)
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
        ...
        describe("#case ClearABIContract", function () {
            ...
            it('should clear contract abi, it should return true', async function () {
                ...
                for (let param of params) {
                    ...
                    // clear abi
                    transaction = await cypherWeb.transactionBuilder.clearABI(...param);
                    const authResult =
                            CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
        ...
        describe("#case AssetIssueContract", function () {
            it(`should allow accounts[2] to create a TestToken`, async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    if (i === 1) options.permissionId = 2;
                    const transaction = await cypherWeb.transactionBuilder.createToken(options, accounts.b58[createrAccountIdx]);
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
        ...
        describe("#case UpdateAssetContract", function () {
            ...
            it(`should allow accounts[3] to update a TestToken`, async function () {
                for (let i = 0; i < 2; i++) {
                    ...
                    const transaction = await cypherWeb.transactionBuilder.updateToken(UPDATED_TEST_TOKEN_OPTIONS, accounts.b58[3]);
                    const authResult = CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
        ...
        describe("#case SetAccountIdContract", function () {
            it(`should set account id accounts[3]`, async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    let accountId = CypherWeb.toHex('abcabc110');
                    let param = [accountId, accounts.b58[3]]
                    if (i === 1) {
                        accountId = CypherWeb.toHex('testtest');
                        param = [accountId, accounts.b58[3], { permissionId: 2 }]
                    }
                    const transaction = await cypherWeb.transactionBuilder.setAccountId(param[0]);
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
        ...
        describe("#case ProposalCreateContract", function () {
            ...
            before(async () => {
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[3], 10000e6), PRIVATE_KEY);
                const transaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[3], 'url.cypher.network');
        ...
        describe("#case ProposalDeleteContract", function () {
            ...
            before(async function () {
                ...
                // const transaction = await cypherWeb.transactionBuilder.applyForSR(ADDRESS_BASE58, 'url.cypher.network');
        ...
        describe("#case ProposalApproveContract", function () {
            ...
            before(async function () {
                ...
                const sendCypTransaction = await cypherWeb.transactionBuilder.sendCyp(accounts.b58[5], 10000e6);
                await broadcaster(sendCypTransaction, PRIVATE_KEY);
                await waitChainData('tx', sendCypTransaction.txID);
                const applyForSrTransaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[5], 'url.cypher.network');
            ...
            it('should allow vote proposal', async function () {
                ...
                for (let param of params) {
                    const transaction = await cypherWeb.transactionBuilder.voteProposal(...param)
                    const authResult =
                        CypherWeb.utils.transaction.txCheck(transaction);
                    assert.equal(authResult, true);
                    commonAssertPb(transaction);
    ...
    describe('#txCheckWithArgs', function () {
        ...
        const commonAssertFalse = (methodName, generateData, params) => {
            it(`it should return false when transaction data is tampered`, async function () {
                for (const param of params) {
                    const transaction = await cypherWeb.transactionBuilder[methodName](
                        ...param
                    );
                    transaction.raw_data_hex = transaction.raw_data_hex + '00';
                    const data = generateData(param);
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction, data, param[3] || {});
        ...
        const commonAssertPbWithArgs = (transaction, args, options) => {
            const transactionPb = CypherWeb.utils.transaction.txJsonToPbWithArgs(transaction, args, options);
            const rawDataBytes = transactionPb.getRawData().serializeBinary();
            const txID = CypherWeb.utils.ethersUtils.sha256(rawDataBytes);
        ...
        describe('#case TransferContract', function () {
            ...
            const generateData = (param) => {
                return {
                    to_address: CypherWeb.address.toHex(param[0]),
                    owner_address: cypherWeb.defaultAddress.hex,
                    amount: param[1],
            ...
            commonAssertFalse('sendCyp', generateData, params);
        });
        ...
        describe('#case TransferAssetContract', function () {
            ...
            const generateData = (param) => {
                return {
                    to_address: cypherWeb.address.toHex(param[0]),
                    owner_address: cypherWeb.address.toHex(param[3]),
                    asset_name: cypherWeb.fromUtf8(param[2]),
            ...
            before(async function () {
                ...
                await broadcaster(cypherWeb.transactionBuilder.createToken(tokenOptions, accounts.b58[13]), accounts.pks[13])

                let tokenList
                await waitChainData('token', accounts.b58[13]);
                tokenList = await cypherWeb.cyp.getTokensIssuedByAddress(accounts.b58[13]);
                if (isAllowSameTokenNameApproved) {
                    tokenID = tokenList[tokenOptions.name].id
                } else {
                    tokenID = tokenList[tokenOptions.name].name
            ...
            it(`it should return true`, async function () {
                ...
                for (let param of params) {
                    ...
                    const data = generateData(param);
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction, data);
                    assert.equal(authResult, true);
        ...
        describe('#case ParticipateAssetIssueContract', function () {
            ...
            const generateData = (param) => {
                return {
                    to_address: cypherWeb.address.toHex(param[0]),
                    owner_address: cypherWeb.address.toHex(param[3]),
                    asset_name: cypherWeb.fromUtf8(param[1]),
            ...
            before(async function () {
                tokenOptions = getTokenOptions();
                tokenOptions.saleEnd += 60 * 60 * 1000;
                await broadcaster(cypherWeb.transactionBuilder.createToken(tokenOptions, accounts.b58[12]), accounts.pks[12])
                await waitChainData('token', accounts.b58[12]);
                let tokenList = await cypherWeb.cyp.getTokensIssuedByAddress(accounts.b58[12]);
                if (isAllowSameTokenNameApproved) {
                    tokenID = tokenList[tokenOptions.name].id
                } else {
                    tokenID = tokenList[tokenOptions.name].name
        ...
        describe('#case TriggerSmartContract', function () {
            ...
            const generateData = (param) => {
                const args = {
                    contract_address: cypherWeb.address.toHex(param[0]),
                ...
                for (let i = 0; i < parameters.length; i++) {
                    ...
                    if (type === 'address')
                        value = cypherWeb.address.toHex(value).replace(/^0x/, '');
                    else if (type.match(/^([^\x5b]*)(\x5b|$)/)[0] === 'address[')
                        value = value.map(v => cypherWeb.address.toHex(v).replace(/^0x/, ''));
                ...
                args.call_value = 0;
                args.fee_limit = cypherWeb.feeLimit;
                return args;
            };
            ...
            it('should trigger smart contract successfully', async function () {
                for (const param of params) {
                    ...
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction.transaction, args);
                    assert.equal(authResult, true);
        ...
        describe.skip('#case UnfreezeBalanceContract', function () {
            ...
            it('it should return true', async function () {
                ...
                for (let param of params) {
                    ...
                    const transaction = await cypherWeb.transactionBuilder.unfreezeBalance(...param)
                    const data = {
                        owner_address: cypherWeb.address.toHex(param[1]),
                        resource: param[0],
                        Permission_id: param[2]?.permissionId,
                    }
                    const authResult =
        ...
        describe.skip('#case WithdrawBalanceContract', function () {
            ...
            it(`it should return true`, async function () {
                ...
                for (let param of params) {
                    ...
                    const data = {
                        owner_address: cypherWeb.address.toHex(param[0]),
        ...
        describe('#case FreezeBalanceV2Contract', function () {
            ...
            const generateData = (param) => {
                return {
                    frozen_balance: parseInt(param[0]),
                    resource: param[1],
                    owner_address: cypherWeb.address.toHex(param[2]),
        ...
        describe('#case DelegateResourceContract', function () {
            ...
            const generateData = (param) => {
                return {
                    ...
                    resource: param[2],
                    owner_address: cypherWeb.address.toHex(param[3]),
                    receiver_address: cypherWeb.address.toHex(param[1]),
                    lock: param[4],
        ...
        describe("#case WitnessCreateContract", async function () {
            ...
            const generateData = (param) => {
                return {
                    owner_address: cypherWeb.address.toHex(param[0]),
                    url: cypherWeb.fromUtf8(param[1]),
                    Permission_id: param[2]?.permissionId,
                };
            };
            ...
            it('should allow accounts[1] to apply for SR, it should return true', async function () {
                for (const param of params) {
                    const sendCypTransaction = cypherWeb.transactionBuilder.sendCyp(param[0], 11000e6);
                    await broadcaster(sendCypTransaction, PRIVATE_KEY);
                    const transaction = await cypherWeb.transactionBuilder.applyForSR(...param);
                    const data = generateData(param);
                    const authResult =
        ...
        describe('#case VoteWitnessContract', function () {
            ...
            const generateData = (param) => {
                return {
                    owner_address: cypherWeb.address.toHex(accounts.b58[1]),
                    votes: Object.entries(param[0]).map(([address, voteCount]) => {
                        return {
                            vote_address: CypherWeb.address.toHex(address),
            ...
            before(async function () {
                ...
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[12], 10000e6), PRIVATE_KEY);
                await broadcaster(cypherWeb.transactionBuilder.applyForSR(accounts.b58[12], url), accounts.pks[12])
                await broadcaster(cypherWeb.transactionBuilder.freezeBalanceV2(100e6, 'BANDWIDTH', accounts.b58[1]), accounts.pks[1])
            })
        ...
        describe("#case CreateSmartContract", function () {
            it('should create a smart contract with default parameters, it should return true', async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction, args, options);
                    assert.equal(authResult, true);
        ...
        describe("#case UpdateBrokerageContract", async function () {
            before(async () => {
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[13], 10000e6), PRIVATE_KEY);
                const transaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[13], 'url.cypher.network');
                await broadcaster(transaction, accounts.pks[13]);
                await waitChainData('tx', transaction.txID);
            });
            it('should auth sr brokerage successful', async function () {
                const transaction = await cypherWeb.transactionBuilder.updateBrokerage(10, accounts.hex[13]);
                const data = {
                    brokerage: 10,
            ...
            it(`should allow accounts[2] to create a TestToken`, async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const {
                        ...
                        url = false,
                        totalSupply = 0,
                        cypRatio = 1, // How much CYP will `tokenRatio` cost?
                        tokenRatio = 1, // How many tokens will `cypRatio` afford?
                        saleStart = Date.now(),
                        saleEnd = false,
                    ...
                    const data = {
                        ...
                        name: cypherWeb.fromUtf8(name),
                        abbr: cypherWeb.fromUtf8(abbreviation),
                        description: cypherWeb.fromUtf8(description),
                        url: cypherWeb.fromUtf8(url),
                        total_supply: parseInt(totalSupply),
                        cyp_num: parseInt(cypRatio),
                    ...
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction, data, options);
                    assert.equal(authResult, true);
        ...
        describe("#case AccountCreateContract", function () {
            ...
            const generateData = (param) => {
                return {
                    account_address: cypherWeb.address.toHex(param[0]),
                    owner_address: cypherWeb.address.toHex(param[1]),
                    Permission_id: param[2]?.permissionId
                };
            };
        ...
        describe("#case AccountUpdateContract", function () {
            ...
            const generateData = (param) => {
                return {
                    account_name: cypherWeb.fromUtf8(param[0]),
                    owner_address: cypherWeb.address.toHex(param[1]),
                    Permission_id: param[2]?.permissionId
                };
            };
        ...
        describe("#case SetAccountIdContract", function () {
            it(`should set account id accounts[3]`, async function () {
                ...
                for (let id of ids) {
                    let accountId = CypherWeb.toHex(id);
                    const transaction = await cypherWeb.transactionBuilder.setAccountId(accountId, accounts.b58[3]);
                    const data = {
        ...
        describe("#case ProposalCreateContract", function () {
            ...
            const generateData = (input) => {
                return {
                    owner_address: cypherWeb.address.toHex(input[1]),
                    parameters: [input[0]],
            ...
            before(async () => {
                ...
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[14], 10000e6), PRIVATE_KEY);
                const transaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[14], 'url.cypher.network');
        ...
        describe("#case ProposalDeleteContract", function () {
            ...
            const generateData = (param) => {
                return {
                    owner_address: cypherWeb.address.toHex(ADDRESS_BASE58),
                    proposal_id: param[0],
                    Permission_id: param[1]?.permissionId,
                };
            };
        ...
        describe("#case ProposalApproveContract", function () {
            ...
            before(async function () {
                ...
                const sendCypTransaction = await cypherWeb.transactionBuilder.sendCyp(accounts.b58[16], 10000e6);
                await broadcaster(sendCypTransaction, PRIVATE_KEY);
                await waitChainData('tx', sendCypTransaction.txID);
                const applyForSrTransaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[16], 'url.cypher.network');
        ...
        describe("#case ExchangeCreateContract", function () {
            ...
            it('should create token exchange', async function () {
                ...
                let data = {
                    ...
                    first_token_id: cypherWeb.fromUtf8(tokenNames[0]),
                    first_token_balance: 10e3,
                    second_token_id: cypherWeb.fromUtf8(tokenNames[1]),
        ...
        describe("#case ExchangeInjectContract", function () {
            ...
            const generateData = (param) => {
                return {
                    ...
                    token_id: cypherWeb.fromUtf8(param[1]),
        ...
        describe("#case AccountPermissionUpdateContract", function () {
            before(async () => {
                await broadcaster(cypherWeb.transactionBuilder.sendCyp(accounts.b58[17], 10000e6), PRIVATE_KEY);
                const transaction = await cypherWeb.transactionBuilder.applyForSR(accounts.b58[17], 'url.cypher.network');
        ...
        describe('#case commonOptions', function () {
            it('should include feeLimit and auth true', async function () {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const transaction = await cypherWeb.transactionBuilder.createSmartContract(data)
                    const args = {
                        owner_address: cypherWeb.defaultAddress.hex,
                        call_value: 0,
                        consume_user_resource_percent: 100,
            ...
            it('should include data and auth true', async function () {
                ...
                for (let param of params) {
                    ...
                    const data = generateData(param);
                    const options = {
                        data: cypherWeb.toHex('111'),
                    };
                    const transaction2 = await cypherWeb.transactionBuilder.addUpdateData(
                        transaction,
                        options.data,
                        'hex',
                    );
                    const authResult =
                        CypherWeb.utils.transaction.txCheckWithArgs(transaction2, data, options);
                    assert.equal(authResult, true);
            ...
            it('should include expiration and auth true', async function () {
                ...
                for (let param of params) {
                    ...
                    const transaction2 = await cypherWeb.transactionBuilder.extendExpiration(
                        transaction,
