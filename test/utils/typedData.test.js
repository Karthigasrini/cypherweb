...
describe('CypherWeb.utils.typedData', function () {
    describe('#EIP-712', function () {
        ...
        tests.forEach((test) => {
            it(`encoding ${test.name}`, function () {
                const encoder = CypherWeb.utils._TypedDataEncoder.from(test.types);
                assert.equal(
                    encoder.primaryType,
                    test.primaryType,
                ...
                assert.equal(
                    CypherWeb.utils._TypedDataEncoder.getPrimaryType(test.types),
                    test.primaryType,
                    'getPrimaryType'
                );
                assert.equal(
                    CypherWeb.utils._TypedDataEncoder.hash(
                        test.domain,
                        test.types,
                        test.data
                    ),
    ...
    describe('#EIP-712 with trcToken', function () {
        ...
        it('should be the correct hash domain', function () {
            assert.equal(
                CypherWeb.utils._TypedDataEncoder.hashDomain(domain),
        ...
        it('should be the correct hash struct', function () {
            assert.equal(
                CypherWeb.utils._TypedDataEncoder.hashStruct(
                    'FromPerson',
                    types,
                    value.from
                ),
            ...
            assert.equal(
                CypherWeb.utils._TypedDataEncoder.hashStruct('Mail', types, value),
        ...
        it('should be the correct hash', function () {
            assert.equal(
                CypherWeb.utils._TypedDataEncoder.hash(domain, types, value),
