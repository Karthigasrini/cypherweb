...
export default class CypherWeb extends EventEmitter {
    ...
    constructor(options = false,
        ...
        [
            ...
            this[key] = CypherWeb[key];
        });
        // for sidechain
        if (typeof sideOptions === 'object' && (sideOptions.fullNode || sideOptions.fullHost)) {
            this.sidechain = new SideChain(sideOptions, CypherWeb, this, privateKey);
        } else {
    ...
    async getFullnodeVersion() {
        try {
            const nodeInfo = await this.cyp.getNodeInfo()
    ...
    static toHex(val) {
        if (utils.isBoolean(val))
            return CypherWeb.fromDecimal(+val);

        if (utils.isBigNumber(val))
            return CypherWeb.fromDecimal(val);

        if (typeof val === 'object')
            return CypherWeb.fromUtf8(JSON.stringify(val));
        ...
        if (utils.isString(val)) {
            ...
            if ((!isFinite(val)) || /^\s*$/.test(val))
                return CypherWeb.fromUtf8(val);
        }

        let result = CypherWeb.fromDecimal(val);
        if (result === '0xNaN') {
    ...
    static toDecimal(value) {
        return CypherWeb.toBigNumber(value).toNumber();
    }

    static fromDecimal(value) {
        const number = CypherWeb.toBigNumber(value);
    ...
    static fromSun(sun) {
        const cyp = CypherWeb.toBigNumber(sun).div(1_000_000);
        return utils.isBigNumber(sun) ? cyp : cyp.toString(10);
    }

    static toSun(cyp) {
        const sun = CypherWeb.toBigNumber(cyp).times(1_000_000);
        return utils.isBigNumber(cyp) ? sun : sun.toString(10);
    }
