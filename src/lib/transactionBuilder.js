...
function fromUtf8(value) {
    return self.cypherWeb.fromUtf8(value).replace(/^0x/, '');
}
...
function resultManager(transaction, data, options, callback) {
    ...
    if (transaction.result && transaction.result.message) {
        return callback(
            self.cypherWeb.toUtf8(transaction.result.message)
        );
    }
...
function genContractAddress(ownerAddress, txID) {
    return '1c' + keccak256(Buffer.from(txID + ownerAddress, 'hex')).toString().substring(2).slice(24);
}
...
async function createTransaction(cypherWeb, type, value, Permission_id, options = {}) {
    const metaData = await getHeaderInfo(cypherWeb.fullNode);
    const tx = {
        visible: false,
        txID: '',
        raw_data_hex: '',
...
export default class TransactionBuilder {
    ...
    sendCyp(to = false, amount = 0, from = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        ...
        if (utils.isFunction(from)) {
            callback = from;
            from = this.cypherWeb.defaultAddress.hex;
        ...
        if (!callback)
            return this.injectPromise(this.sendCyp, to, amount, from, options);
        ...
        if (this.validator.notValid([
            ...
            },
            {
                ...
                type: 'notEqual',
                msg: 'Cannot transfer CYP to the same account'
            },
            {
                name: 'amount',
                type: 'integer',
        ...
        createTransaction(this.cypherWeb, 'TransferContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    sendToken(to = false, amount = 0, tokenID = false, from = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'TransferAssetContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    purchaseToken(issuerAddress = false, tokenID = false, amount = 0, buyer = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(buyer)) {
            callback = buyer;
            buyer = this.cypherWeb.defaultAddress.hex;
        ...
        createTransaction(this.cypherWeb, 'ParticipateAssetIssueContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    freezeBalance(amount = 0, duration = 3, resource = "BANDWIDTH", address = this.cypherWeb.defaultAddress.hex, receiverAddress = undefined, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'FreezeBalanceContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    unfreezeBalance(resource = "BANDWIDTH", address = this.cypherWeb.defaultAddress.hex, receiverAddress = undefined, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'UnfreezeBalanceContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    freezeBalanceV2(amount = 0, resource = "BANDWIDTH", address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'FreezeBalanceV2Contract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    unfreezeBalanceV2(amount = 0, resource = "BANDWIDTH", address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'UnfreezeBalanceV2Contract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    cancelUnfreezeBalanceV2(address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'CancelAllUnfreezeV2Contract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    delegateResource(
        ...
        createTransaction(this.cypherWeb, 'DelegateResourceContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    undelegateResource(amount = 0, receiverAddress, resource = "BANDWIDTH", address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'UnDelegateResourceContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    withdrawExpireUnfreeze(address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'WithdrawExpireUnfreezeContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    withdrawBlockRewards(address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'WithdrawBalanceContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    applyForSR(
        ...
        createTransaction(this.cypherWeb, 'WitnessCreateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    vote(votes = {}, voterAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(voterAddress)) {
            ...
            voterAddress = this.cypherWeb.defaultAddress.hex;
        ...
        createTransaction(this.cypherWeb, 'VoteWitnessContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    createSmartContract(options = {}, issuerAddress = this.cypherWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.cypherWeb.defaultAddress.hex;
        }

        if (!callback)
            return this.injectPromise(this.createSmartContract, options, issuerAddress);

        const feeLimit = options.feeLimit || this.cypherWeb.feeLimit;
        ...
        // tokenValue and tokenId can cause errors if provided when the cyp10 proposal has not been approved yet. So we set them only if they are passed to the method.
        ...
        new Promise((resolve) => {
            ...
            .then(async contract => {
                const tx = await createTransaction(this.cypherWeb, 'CreateSmartContract', contract, options?.permissionId, { fee_limit: args.fee_limit })
    ...
    async deployConstantContract(options = {}) {
        ...
        return this.cypherWeb[options.confirmed ? 'solidityNode' : 'fullNode']
            ...
            .then(transaction => {
                ...
                if (transaction.result && transaction.result.message) {
                    throw new Error(
                        this.cypherWeb.toUtf8(transaction.result.message)
                    );
                }
                return transaction;
            })
    ...
    _triggerSmartContractLocal(
        ...
        let {
            ...
            callValue: 0,
            feeLimit: this.cypherWeb.feeLimit
        }, options)
    ...
    _triggerSmartContract(
        ...
        this.cypherWeb[options.confirmed ? 'solidityNode' : 'fullNode'].request(pathInfo, args, 'post').then(transaction => resultManagerTriggerSmartContract(transaction, args, options, callback)).catch(err => callback(err));
    }

    clearABI(contractAddress, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {     
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (utils.isFunction(ownerAddress)) {
            ...
            ownerAddress = this.cypherWeb.defaultAddress.hex;
        ...
        if (!this.cypherWeb.isAddress(ownerAddress))
        ...
        if (this.cypherWeb.cyp.cache.contracts[contractAddress]) {
            delete this.cypherWeb.cyp.cache.contracts[contractAddress]
        }

        createTransaction(this.cypherWeb, 'ClearABIContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));

    }

    updateBrokerage(brokerage, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'UpdateBrokerageContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    createToken(options = {}, issuerAddress = this.cypherWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.cypherWeb.defaultAddress.hex;
        }
        ...
        createTransaction(this.cypherWeb, 'AssetIssueContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    createAccount(accountAddress, address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'AccountCreateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    updateAccount(accountName = false, address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'AccountUpdateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    setAccountId(accountId, address = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'SetAccountIdContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    updateToken(options = {}, issuerAddress = this.cypherWeb.defaultAddress.hex, callback = false) {
        if (utils.isFunction(issuerAddress)) {
            callback = issuerAddress;
            issuerAddress = this.cypherWeb.defaultAddress.hex;
        ...
        createTransaction(this.cypherWeb, 'UpdateAssetContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
     ...
     */
    createProposal(parameters = false, issuerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'ProposalCreateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
     ...
     */
    deleteProposal(proposalID = false, issuerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'ProposalDeleteContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
     ...
     * Only current Super Representative can vote on a proposal.
     */
    voteProposal(proposalID = false, isApproval = false, voterAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'ProposalApproveContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    createCYPExchange(tokenName, tokenBalance, cypBalance, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        if (!callback)
            return this.injectPromise(this.createCYPExchange, tokenName, tokenBalance, cypBalance, ownerAddress, options);
        ...
        const data = {
            ...
            second_token_id: '5f', // Constant for CYP.
            second_token_balance: cypBalance
        };

        createTransaction(this.cypherWeb, 'ExchangeCreateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }

    createTokenExchange(firstTokenName, firstTokenBalance, secondTokenName, secondTokenBalance, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
     ...
     * Use "_" for the constant value for CYP.
     */
    injectExchangeTokens(exchangeID = false, tokenName = false, tokenAmount = 0, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'ExchangeInjectContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
     ...
     * Use "_" for the constant value for CYP.
     */
    withdrawExchangeTokens(exchangeID = false, tokenName = false, tokenAmount = 0, ownerAddress = this.cypherWeb.defaultAddress.hex, options, callback = false) {
        if (utils.isFunction(options)) {
            callback = options;
            options = {};
        }
        ...
        createTransaction(this.cypherWeb, 'ExchangeWithdrawContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    tradeExchangeTokens(exchangeID = false,
        ...
        const data = {
            owner_address: toHex(ownerAddress),
            exchange_id: parseInt(exchangeID),
            token_id: this.cypherWeb.fromAscii(tokenName).replace(/^0x/, ''),
            quant: parseInt(tokenAmountSold),
            expected: parseInt(tokenAmountExpected)
        };

        createTransaction(this.cypherWeb, 'ExchangeTransactionContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    updateSetting(contractAddress = false,
        ...
        createTransaction(this.cypherWeb, 'UpdateSettingContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    updateEnergyLimit(contractAddress = false,
        ...
        createTransaction(this.cypherWeb, 'UpdateEnergyLimitContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    checkPermissions(permissions, type) {
        if (permissions) {
            ...
            for (let key of permissions.keys) {
                if (!this.cypherWeb.isAddress(key.address)
    ...
    updateAccountPermissions(
        ...
        createTransaction(this.cypherWeb, 'AccountPermissionUpdateContract', data, options?.permissionId)
            .then(transaction => callback(null, transaction))
            .catch(err => callback(err));
    }
    ...
    async newTxID(transaction, options, callback) {
        ...
        if (options?.txLocal) {
            ...
            createTransaction(this.cypherWeb, contract.type, contract.parameter.value, contract.Permission_id, {
    ...
    async alterTransaction(transaction, options = {}, callback = false) {
        ...
        if (options.data) {
            if (options.dataFormat !== 'hex')
                options.data = this.cypherWeb.toHex(options.data);
