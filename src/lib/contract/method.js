...
export default class Method {
    constructor(contract, abi) {
        this.cypherWeb = contract.cypherWeb;
        ...
        this.signature = this.cypherWeb.sha3(this.functionSelector, false).slice(0, 8);
    ...
    onMethod(...args) {
      ...
      return {
          ...
          send: (options = {}, privateKey = this.cypherWeb.defaultPrivateKey, callback = false) => {
    ...
    async _call(types, args, options = {}, callback = false) {
        ...
        this.cypherWeb.transactionBuilder.triggerSmartContract(
            this.contract.address,
            this.functionSelector,
            options,
            parameters,
            options.from ? this.cypherWeb.address.toHex(options.from) : false,
            (err, transaction) => {
                if (err)
                    return callback(err);
                    ...
                    if (len === 0 || len % 64 === 8) {
                        ...
                        if (len !== 0) {
                            ...
                            for (let i = 0; i < len - 8; i += 64) {
                                msg2 += this.cypherWeb.toUtf8(chunk.substring(i, i + 64))
                            }
    ...
    async _send(types, args, options = {}, privateKey = this.cypherWeb.defaultPrivateKey, callback = false) {
        if (utils.isFunction(privateKey)) {
            callback = privateKey;
            privateKey = this.cypherWeb.defaultPrivateKey;
        }
        ...
        try {
            const address = privateKey ? this.cypherWeb.address.fromPrivateKey(privateKey) : this.cypherWeb.defaultAddress.base58;
            const transaction = await this.cypherWeb.transactionBuilder.triggerSmartContract(
                this.contract.address,
                this.functionSelector,
                options,
                parameters,
                this.cypherWeb.address.toHex(address)
            );
            ...
            const signedTransaction = await this.cypherWeb.cyp.sign(transaction.transaction, privateKey);
            ...
            const broadcast = await this.cypherWeb.cyp.sendRawTransaction(signedTransaction);
            ...
            if (broadcast.code) {
                ...
                if (broadcast.message)
                    err.message = this.cypherWeb.toUtf8(broadcast.message);
                return callback(err)
            }
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.cypherWeb.cyp.getTransactionInfo(signedTransaction.txID);
                ...
                if (output.result && output.result === 'FAILED') {
                    return callback({
                        error: this.cypherWeb.toUtf8(output.resMessage),
                        transaction: signedTransaction,
                        output
                    });
                }
    ...
    async _watch(options = {}, callback = false) {
        ...
        const getEvents = async () => {
            try {
                ...
                const events = await this.cypherWeb.event.getEventsByContractAddress(this.contract.address, params);
