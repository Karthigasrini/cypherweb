...
export default class SideChain {
    constructor(sideOptions, CypherWeb = false, mainchain = false, privateKey = false) {
        this.mainchain = mainchain;
        const { fullHost, fullNode, solidityNode, eventServer, mainGatewayAddress, sideGatewayAddress, sideChainId } = sideOptions;
        this.sidechain = new CypherWeb(fullHost || fullNode, fullHost || solidityNode, fullHost || eventServer, privateKey);
        ...
        const self = this;
        this.sidechain.cyp.sign = (...args) => {
            return self.sign(...args);
        };
        this.sidechain.cyp.multiSign = (...args) => {
            return self.multiSign(...args);
        };

        console.warn("CypherWeb: 'cypherWeb.sidechain' is deprecated and may be removed in the future. Please use the 'sunweb' sdk instead. For more information, see: https://github.com/tronprotocol/sun-network/tree/develop/js-sdk");
    }
    ...
    async multiSign(transaction = false, privateKey = this.sidechain.defaultPrivateKey, permissionId = false, callback = false) {
        ...
        if (!transaction.raw_data.contract[0].Permission_id && permissionId > 0) {
            ...
            const signWeight = await this.sidechain.cyp.getSignWeight(transaction, permissionId);
    ...
    async sign(transaction = false, privateKey = this.sidechain.defaultPrivateKey, useCypherHeader = true, multisig = false, callback = false) {
        if (this.utils.isFunction(multisig)) {
            callback = multisig;
            multisig = false;
        }

        if (this.utils.isFunction(useCypherHeader)) {
        ...
        if (this.utils.isString(transaction)) {
            ...
            try {
                const signatureHex = this.sidechain.cyp.signString(transaction, privateKey, useCypherHeader);
    ...
    */
    async depositCyp(
        ...
        if (!callback) {
            return this.injectPromise(this.depositCyp, callValue, depositFee, feeLimit, options, privateKey);
        }
        ...
        try {
            ...
            const result = await contractInstance.depositCYP().send(options, privateKey);
    ...
    async mappingTrc(
        ...
        if (!callback) {
            return this.injectPromise(this.mappingTrc, cypHash, mappingFee, feeLimit, functionSelector, options, privateKey);
        }
        if (this.validator.notValid([
            {
                name: 'cypHash',
        ...
        cypHash = cypHash.startsWith('0x') ? cypHash : ('0x' + cypHash);
        options = {
            feeLimit,
            ...options,
        ...
        try {
            ...
            if (functionSelector === 'mappingTRC20') {
                result = await contractInstance.mappingTRC20(cypHash).send(options, privateKey);
            } else if (functionSelector === 'mappingTRC721') {
                result = await contractInstance.mappingTRC721(cypHash).send(options, privateKey);
            } else {
    ...
    /**
     * withdraw cyp from sidechain to mainchain
     */
    async withdrawCyp(
        callValue,
        withdrawFee,
        ...
        if (!callback) {
            return this.injectPromise(this.withdrawCyp, callValue, withdrawFee, feeLimit, options, privateKey);
        }
        if (this.validator.notValid([
            {
                name: 'callValue',
        ...
        try {
            ...
            const result = await contractInstance.withdrawCYP().send(options, privateKey);
            return callback(null, result);
        } catch (ex) {
            return callback(ex);
        }
    ...
    async withdrawTrc(
        ...
        try {
            ...
            const signedTransaction = await this.sidechain.cyp.sign(transaction.transaction, privateKey);
            ...
            const broadcast = await this.sidechain.cyp.sendRawTransaction(signedTransaction);
            if (broadcast.code) {
                const err = {
            ...
            const checkResult = async (index = 0) => {
                ...
                const output = await this.sidechain.cyp.getTransactionInfo(signedTransaction.txID);
    ...
    async injectFund(
        ...
        try {
            ...
            const signedTransaction = await this.sidechain.cyp.sign(transaction, privateKey);
