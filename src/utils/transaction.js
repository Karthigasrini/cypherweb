...
const { Transaction, Permission, Key } = require('../protocol/core/Cypher_pb');
...
const fromHexString = (hexString) => {
    ...
    return new Uint8Array(
        CypherWeb.address.toHex(hexString).match(/.{1,2}/g).map((byte) => parseInt(byte, 16))
    );
};
...
// wallet/createtransaction for sendCyp
...
const buildAssetIssueContract = (value, options) => {
    ...
    assetIssueContract.setCypNum(cyp_num);
