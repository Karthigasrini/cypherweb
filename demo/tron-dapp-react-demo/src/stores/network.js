...
export default class NetworkStore {
  @observable cypherWeb = false;
  @observable defaultAccount = null; // current login cypher account address
  ...
  checkLogin = () => {
    if (!this.cypherWeb || !this.cypherWeb.defaultAddress.base58) {
      return false;
    }
  ...
  initCypherLinkWallet = async (cb, cbn) => {
    try {
      let timeCount = 0;
      const self = this;
      const tmpTimer1 = setInterval(() => {
        ...
        if (window.cypherWeb && window.cypherWeb.ready) {
          if (process.env.REACT_APP_ENV === 'test' || process.env.REACT_APP_ENV === 'qaTest') {
            window.cypherWeb.setFullNode(Config.chain.fullHost);
            window.cypherWeb.setSolidityNode(Config.chain.fullHost);
          }
          const { trongrid } = Config;

          if (trongrid && window.cypherWeb.setHeader && window.cypherWeb.fullNode.host === trongrid.host) {
            window.cypherWeb.setHeader({ 'CYPHER-PRO-API-KEY': trongrid.key });
          }
          self.cypherWeb = window.cypherWeb;
          self.defaultAccount = self.cypherWeb.defaultAddress.base58;
